<?php
    include "db_config.php";
        class user
        {
            ...
            public function __construct()
            {
                ...
                {
                    echo "Error: ไม่สามารถเชื่อต่อฐานจข้อมูลได้่.";
                    exit;
                }
            }
            public function reg_user($name, $username, $password, $email)
            {
                ...
                if($count_row==0)
                {
                    ...
                    $result= mysqli_query($this->db,$sql1) or die(mysqli_connect_errno()."ไม่สามารถแทรกข้อมูลได้");
                    return $result;
                }
                else
                {
            ...
            public function add_room($roomname, $room_qnty, $no_bed, $bedtype,$facility,$price)
            {
                    ...
                    $result= mysqli_query($this->db,$sql) or die(mysqli_connect_errno()."ไม่สามารถแทรกข้อมูลได้");
   ...
   FROM rooms WHERE (checkin <= '$checkin' AND checkout >='$checkout') OR (checkin >= '$checkin' AND checkin <='$checkout') OR (checkin <= '$checkin' AND checkout >='$checkin') )";
                    $check= mysqli_query($this->db,$sql)  or die(mysqli_connect_errno()."Query ไม่ทำงาน");;
   ...
   FROM rooms WHERE checkin <= '$checkin' AND checkout >='$checkout'))";
                    $check= mysqli_query($this->db,$sql)  or die(mysqli_connect_errno()."ไม่สามารถแทรกข้อมูลได้");;
                    ...
                    if(mysqli_num_rows($check) > 0)
                    {
                        ...
                        if($send)
                        {
                            $result="จองห้องพักสำเร็จแล้ว!!";
                        }
                        else
                        {
                            $result="ขออภัย, อินเทอร์เน็ต Error";
                        }
                    }
                    else                       
                    {
                        $result = "ไม่มีห้องว่าง";
                    }
             ...
             public function edit_all_room($checkin, $checkout, $name, $phone,$id)
            {
                        ...
                        if($send)
                        {
                            $result="ห้องของคุณได้ถูกจองแล้ว!!";
                        }
                        else
                        {
                            $result="ขออภัย, อินเทอร์เน็ต Error";
